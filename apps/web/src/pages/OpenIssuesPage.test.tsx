...
// legacy client id the route carries — OpenIssuesPage now resolves the
...
describe('OpenIssuesPage', () => {
  ...
  // issues". The screen must now surface it as its own soon-severity issue in
  // the same list the customer is already reading (see OpenIssuesPage.tsx's
  // `healthError` branch).
  it('surfaces a health-load failure as an issue instead of swallowing it', async () => {
    mockGetCaseHealth.mockRejectedValue(new Error('network error'));
    renderPage('client-demo-001');
    expect(
      await screen.findByText('טעינת נתוני התיק נכשלה. שום נתון בתיק לא נפגע.'),
    ).toBeInTheDocument();
  });
