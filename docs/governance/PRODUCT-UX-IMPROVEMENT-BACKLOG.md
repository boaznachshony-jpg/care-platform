...
**עדכון אחרון:** 03.09.2026 — Reconciliation שני מול הקוד לאחר סבב ה-QA המלא (PR #120 נפרס לייצור, PR #122 מוכן למיזוג). נוספו R0-10, R1-07, R1-08, R2-08, R5-08, R7-09.  
...
| R0-09 | הגלאי מזהה — ואף אחד לא שומע | **PRODUCTION_VERIFIED** | **P0** | `packages/application/src/ports/data-loss-alert-sink.ts` (הפורט מצהיר על הפער) · `packages/infrastructure/src/monitoring/logging-data-loss-alert-sink.ts` (שורת לוג בלבד) · **חדש:** `apps/api/src/monitoring/email-data-loss-alert-sink.ts` + חיווט ב-`container.ts` + `DATA_LOSS_ALERT_EMAIL` ב-`env.ts` · PR #117 | 01.09.2026 — **נפתח בעקבות דרישת הבעלים לאפס אובדן נתונים, ונסגר באותו יום.** הסריקה היומית מזהה התכווצות נתונים מאז מיגרציה 0038, אבל הסימן `DATA_LOSS_SUSPECTED` נכתב לשורת לוג ב-Vercel שאיש אינו מנוי אליה — הבקרה בפועל הייתה "שמישהו ייזכר לפתוח לוגים". **מה שנוסף:** יעד מייל דרך Resend, שהוא כבר גבול הדואר היחיד במוצר. **שורת הלוג נכתבת ראשונה ותמיד**, כדי שהראיה לא תלויה בזמינות Resend. כשל שליחה **אינו נזרק** — הוא היה עוצר את הסריקה ומשאיר דיירים מאוחרים יותר לא נמדדים — אלא נרשם תחת `DATA_LOSS_ALERT_UNDELIVERED`, כי "זיהינו ולא הצלחנו לספר" הוא אירוע תפעולי בפני עצמו. המייל נושא מזהה דייר, שם מדד ושני מספרים בלבד — **אפס נתוני לקוח** — ושלושה צעדים לפעולה כולל צוק שבעת הימים. `env.ts` דוחה יעד ללא transport; `/ready` בייצור מדווח כסיבה כשהיעד חסר. **7 בדיקות, הורצו ועברו.** **תלוי בפעולה תפעולית: הגדרת `DATA_LOSS_ALERT_EMAIL` ב-Vercel + Redeploy.** **03.09.2026 — אומת בייצור שהפעולה בוצעה, ותיקון לרישום שגוי שלי.** `GET https://care-platform-api.vercel.app/ready` מחזיר `{"status":"ok"}` ללא `reasons`; `container.ts:822` מוסיף במפורש `reasons.push('DATA_LOSS_ALERT_EMAIL is not configured…')` בייצור כשהמשתנה חסר, ולכן היעדר הסיבה הוא **ראיה חיובית** שהוא מוגדר. עד היום המשכתי לדווח שהמשתנה חסר על סמך הרישום מ-01.09, בלי לבדוק — **דיווח לא מאומת שחזרתי עליו יותר מפעם אחת.** הפריט עולה ל-`PRODUCTION_VERIFIED`. מה שנשאר פתוח הוא **מעטפת ההתאוששות** (PITR), לא ערוץ ההתראה. |

| R0-11 | PITR — הוחלט **לא** לרכוש כרגע, ובמקומו גיבוי לוגי יומי | **LATER** | P0 | תמחור Supabase: **$100 לחודש לכל 7 ימי שמירה**, ובנוסף PITR מחייב `Small` compute במקום ה-`Micro` הנוכחי · `docs/governance/RESTORE-DRILL.md` · מיגרציה `0038` (הכותרת מתעדת שהשמירה היא 7 ימים מתגלגלים ו-PITR כבוי) | 03.09.2026 — **החלטת הבעלים, BN.** אומת מול פרויקט הייצור `CareDesk Production` (Healthy, Frankfurt, גיבוי אחרון לפני 7 שעות). **הנימוק שנרשם:** $100 לחודש עבור לקוחה אחת עם 17KB אינו פרופורציונלי; המחיר יהפוך סביר כשתהיה הכנסה שמכסה אותו. **מה שכן קיים ולא משתנה:** גיבוי יומי של Supabase לשבעה ימים, וכן `tenant_workspace_history` עם API שחזור שכבר **עבר תרגיל בהצלחה** (29 ערכים שוחזרו) — יחד הם מכסים את התרחיש הסביר: מחיקה בטעות, פריסה גרועה, באג שכתב זבל. **מה שנשאר חשוף:** (1) אובדן שקט שמתגלה אחרי שבעה ימים; (2) אובדן הפרויקט או החשבון כולו. **מה שמקטין את (1) כבר היום:** הסריקה היומית + מייל ההתראה (R0-09) — כשמגלים תוך יום, חלון של שבוע מספיק. **התלות הזו היא בדיוק מה שהופך את R0-12 לקריטי.** **נקודת החלטה מחדש:** כשיהיו ~10 לקוחות משלמים. |
| R0-12 | הגלאי מעולם לא נצפה שולח — והוא מה שמחזיק את חלון השבעה ימים | **NEW** | **P0** | `apps/api/vercel.json` — cron `/internal/jobs/data-integrity-scan` ב-`42 3 * * *` · `apps/api/src/routes/data-integrity.ts:21` · `apps/api/src/monitoring/email-data-loss-alert-sink.ts:60` · `packages/application/src/use-cases/scan-for-data-loss.ts:43` | 03.09.2026 — **נפתח כתוצאה ישירה מהחלטת R0-11.** כל עוד PITR כבוי, ההגנה נשענת על **גילוי מהיר**: הסריקה רצה כל לילה ושולחת מייל אם נתונים התכווצו. היעד מוגדר ואומת (R0-09), אבל **מעולם לא נצפה מייל אמיתי יוצא**. שקט יכול להיות שני דברים הפוכים: אין תקלות, או שהסריקה כלל אינה רצה — ובמקרה השני חלון השבעה ימים הוא אשליה והבעלים מאמין שהוא מוגן. **נדרשת בדיקה חיה:** להפעיל את המסלול ידנית מול דייר סינתטי, לגרום להתכווצות מכוונת, ולראות את המייל מגיע. אין לסמוך על העדר שגיאות כראיה. |
| R0-13 | קיים יותר מפרויקט Supabase אחד, וברירת המחדל אינה הייצור | **NEW** | P1 | פרויקט הייצור: ה-ref שמופיע ב-bundle החי של `care-platform-web.vercel.app` · פרויקט נוסף בחשבון שאליו הבעלים הגיע ראשון | 03.09.2026 — **נמצא בזמן ניסיון להפעיל PITR: הבעלים פתח פרויקט אחר, ורק אימות מול הקוד החי מנע הפעלת תוספת בתשלום על המסד הלא נכון.** אותה טעות עם פקודת SQL הייתה חמורה בהרבה. **פעולה:** לזהות מה הפרויקט הנוסף — אם הוא שריד ניסיוני, לשנות לו שם ל-`DO-NOT-USE` או למחוק; אם הוא מיועד לשימוש, ייתכן שהוא ה-staging החסר של R0-01. **כלל שנקבע:** לפני כל פעולה על Supabase מאמתים את ה-ref מול ה-bundle החי, לא מהזיכרון ולא משם הפרויקט. |
| R0-10 | מסמך שנערך אחרי הסנכרון אינו יכול להגיע לשרת | **NEW** | **P0** | `apps/api/src/routes/case-documents.ts` — רשומים `GET /cases/:caseId/documents` (:120), `POST` (:132), `POST …/import` (:162), `GET …/:documentId/download-url` (:184), `POST`/`GET …/intake-reviews` (:219, :332). **אין `PATCH`, אין `PUT`, אין `DELETE` על מסמך בודד** · אין `updateCaseDocument` בשום מקום ב-`apps/web/src` · ההשלכה מתועדת בקוד עצמו: `apps/web/src/pages/DocumentsPage.tsx:202-208` | 03.09.2026 — **זהו שורש 3 (dual truth) חוזר במסלול פעיל, ולכן P0 ולא P1.** מסך המסמכים מציג כפתורי "עריכה" ו-"מחיקה" (`DocumentsPage.tsx:513`, `:520`) שפועלים **על העותק המקומי בלבד**. אחרי שמסמך יובא לשרת, כל עריכה נוספת נשארת בדפדפן ואין לה נתיב לשרת — כלומר שתי גרסאות של אותו מסמך, והמשתמש אינו יודע איזו מהן "האמת". הסנכרון תוקן במכוון כך ש**המקומי תמיד מנצח** (`apps/web/src/sync/document-mapping.ts`), כדי שהעריכה לא תידרס — אבל זה פתרון זמני שמנציח את הפער, לא סוגר אותו. **מחיקה חמורה יותר**: `deleteDocumentFile` מנקה את הקובץ המקומי ומשאיר את הרשומה בשרת. **פעולה נדרשת:** `PATCH` ו-`DELETE` על `/cases/:caseId/documents/:documentId`, ואז הסרת נתיב הכתיבה המקומי. **חוסם את הסרת האחסון המקומי** (ראה R0-04). **03.09.2026 — לא נסגר במכוון בסבב הזה:** זו עבודת שרת (מסלול, use-case, הרשאות, audit) ולכן Release נפרד, לפי §1 ו-§3 של נוהל השחרור. |

**Gate:** אין מעבר ל-R1 אם קיימת סכנת dual truth, migration לא אמינה או restore לא מוכח.
**מצב השער נכון ל-03.09.2026: נעקף במפורש בהחלטת הבעלים, בהיקף מוגבל.** מותרת עבודת תצוגה ותשתית שאינה נוגעת בנתוני לקוח; **R0-04 חסום** עד תרגיל שחזור מוכח. ראה R0-05. **מאז 02.09 המצב השתנה לטובה בהיקף מוגדר:** משימות, מסמכים ותרופות עברו לאחסון קנוני בשרת (PR #120, מיגרציות 0046–0048), כלומר שלושה מסלולים שהיו browser-only הפסיקו להיות כאלה. זה **אינו** סוגר את R0-04: `mvp-storage.ts` עדיין נתיב כתיבה פעיל, והמסמכים עדיין חסרי endpoint עדכון (R0-10).
...
| ID | נושא | Status | Priority | Evidence / Reference | Notes |
|---|---|---|---|---|---|
| R1-01 | Unified Saving / Saved / Error | PARTIAL | P0 | **שורש 5** · `apps/web/src/pages/PayrollPage.tsx:555` · `apps/web/src/pages/case/AutomationPanel.tsx:62` · `apps/web/src/pages/case/CanonicalPayrollIntelligence.tsx:156` · `apps/web/src/components/PayrollIntelligence.tsx:41-42` · `apps/web/src/storage/workspace-sync.ts:14` | 01.09.2026 — מצב `saving/saved/error` קיים ב-**ארבעה** מסכים בלבד, כל אחד עם `useState` משלו. שורש 5 דורש "חוזה כתיבה אחד לכל האפליקציה, מיושם פעם אחת ומוחל בכל מקום" — הקומפוננטה/hook המשותפים אינם קיימים (`apps/web/src/hooks/` מכיל שלושה hooks, אף אחד מהם אינו hook כתיבה). **03.09.2026 — נמדד מחדש והמצב גרוע מכפי שנרשם: 16 מסכים ופאנלים** מנהלים מצב כתיבה משלהם, ב-**ארבעה אוצרי מילים שונים**: `'idle'\|'saving'\|'saved'\|'error'` (PayrollPage, AutomationPanel), `'idle'\|'loading'\|'saving'\|'saved'\|'conflict'` — **ללא `'error'`** (CanonicalPayrollIntelligence:156), שני בוליאנים (`PayrollIntelligence:41-42`), ומחרוזת חופשית אחת שמשמשת גם להצלחה וגם לכשל (`DocumentsPage.tsx:118`). חיפוש אחר `useSaveState\|useMutation\|SaveStatus` ב-`apps/web/src` מחזיר **אפס תוצאות**. |
| R1-02 | Double-submit protection | PARTIAL | P0 | **שורש 5** · `apps/web/src/pages/BillingPage.tsx:41,272,320,405` · `apps/web/src/pages/case/ProductCompletionPanel.tsx:40,135,241` · `apps/web/src/components/RegulationRulesAdmin.tsx:54,245` | 01.09.2026 — דגלי `busy` לכל מסך בנפרד, כולל בזרימת החיוב. אין מנגנון חוצה-אפליקציה, ולא אומת שהעלאת מסמך מוגנת (WEB-08 "הקשה כפולה יוצרת שני מסמכים" — לא הצלחתי לאתר הגנה במסלול ההעלאה). **03.09.2026 — הפער של המסמכים אומת והופרד לפריט משלו: ראה R1-07.** |
...
| R1-05 | Failure / retry model | PARTIAL | P0 | **= שורש 5 (כתיבה שקטה) + שורש 9 (אין error boundary)** · `apps/web/src/components/ErrorBoundary.tsx` · `apps/web/src/main.tsx:23` (`AppErrorBoundary`) · `apps/web/src/AppShell.tsx:268` (`SectionErrorBoundary resetKey={location.pathname}`) · `apps/web/src/components/ErrorBoundary.test.tsx` · `c8b4175` | 01.09.2026 — **זהו הפריט שהוא בפועל שורש 5 של תוכנית ה-Remediation, ואסור לנהל אותו כרשימה נפרדת.** ה-boundary נחת בשתי רמות (אפליקציה + מקטע, עם איפוס בניווט) — זה חצי שורש 9. חצי שורש 5 — "אין silent failure" ו-**retry** — לא נחת: לא אותר נתיב retry בשום מקום ב-`apps/web/src`. **03.09.2026 — תיקון לרישום הקודם:** קיימים **18 כפתורי "נסו שוב" ידניים**, כל אחד כתוב מחדש (AppShell:179, CaseDocumentsSection:225, CaseTasksSection:153, TasksPage:363/374, MedicationsPage:303/314, DocumentsPage:372, VisaRenewalSection:293/420, ProductCompletionPanel:258/313/403, CollaborationPanel:88, BillingPage:339, FamilyAccessPage:148, LoginPage:437, CasePage:75). מה שאין: **retry אוטומטי** — ל-`apiRequest` (`client.ts:129`) אין לולאת ניסיון, וחיפוש `backoff` מחזיר אפס. ואין הבחנה בין כשל שניתן לנסות שוב לכשל סופי, מלבד בדיקות `ApiRequestError.status` נקודתיות בכל מסך בנפרד. |
| R1-06 | Preservation of user input | PARTIAL | P1 | **שורש 9** · `apps/web/src/storage/form-draft-store.ts` · `DraftStorageError` (WEB-06 — כתיבת localStorage לא מוגנת הפילה את העץ) | 01.09.2026 — עלה מ-`NEW`: התשתית קיימת וגנרית (`saveFormDraft<T>`). מחוברת **רק** ל-`PayrollPage`. טפסים ארוכים אחרים (onboarding, ויזה, מסמכים) לא אומתו כמחוברים. **03.09.2026 — נמדד:** 2 מתוך 5 טפסים ארוכים מכוסים, ובשני מנגנונים שונים שאינם מדברים זה עם זה. אשף השכר משתמש ב-`form-draft-store` (`PayrollPage.tsx:531-533`, `:621-636`); **onboarding משתמש במנגנון ישן ונפרד** — `readMvpOnboardingDraft`/`saveMvpOnboardingDraft` מ-`mvp-storage.js` (`OnboardingPage.tsx:19-21`, `:149-163`, `:181-189`), ולכן אינו מוגן על ידי ההצפנה, ה-TTL ו-`clearAllFormDrafts` של החנות החדשה. **לא מכוסים כלל:** חידוש ויזה (`VisaRenewalSection.tsx` — RHF בזיכרון בלבד; ניווט מאבד שישה שדות), מסמכים (`DocumentsPage.tsx:114-117`), אנשי קשר (`CaseContactsSection.tsx:65-66`). **המסקנה שנרשמת:** "שימור קלט" אינו תכונה של הפלטפורמה אלא שני מימושים חד-פעמיים. |
| R1-07 | העלאת מסמך אינה מוגנת מהקשה כפולה — ואין לה idempotency-key | **DEPLOYED** | **P0** | `apps/web/src/pages/DocumentsPage.tsx:257` (`saveDocument`), `:274` (`crypto.randomUUID()` חדש בכל לחיצה), `:275` (`await saveDocumentFile`), `:458-460` — כפתור השליחה **ללא `disabled`** · מנגד `apps/web/src/pages/case/CaseDocumentsSection.tsx:330` — `disabled={isSubmitting}` · `apps/web/src/api/client.ts:353-361` — `uploadCaseDocument` **אינו שולח `idempotency-key`** | 03.09.2026 — **זהו WEB-08 שנרשם ב-R1-02 כ"לא הצלחתי לאתר הגנה", ועכשיו נמדד ואומת: אין הגנה.** שתי הקשות במהלך ההמתנה מייצרות שני מזהים ולכן שתי רשומות מסמך — לא כפילות שניתן לאחד, אלא שני מסמכים שונים בתיק. מסך המסמכים בתוך התיק (`CaseDocumentsSection`) כן מוגן; המסך העצמאי, שהוא זה שהלקוח משתמש בו בפועל, אינו. **גם עם `disabled` זה לא מספיק**: בהיעדר `idempotency-key`, תשובה שאבדה ברשת + ניסיון חוזר של המשתמש עדיין יוצרים כפילות בשרת. הופרד מ-R1-02 כי הוא ניתן לתיקון בפני עצמו ואינו מחכה לחוזה הכתיבה המשותף. |
| R1-08 | `crypto.randomUUID()` גולמי — הקריסה שכבר קרתה פעם, בחמישה מקומות נוספים | **DEPLOYED** | P1 | `apps/web/src/api/idempotency.ts:2-8` (התיעוד של הכשל) · `apps/web/src/components/PayrollIntelligence.tsx:43` · `apps/web/src/pages/case/CanonicalPayrollIntelligence.tsx:380, 454, 466, 484` · `apps/web/src/pages/WorkerPortalPage.tsx:215, 272` | 03.09.2026 — `crypto.randomUUID` אינו קיים מחוץ ל-secure context; זו בדיוק הקריסה שהעלימה את מסך הכריכה. `newIdempotencyKey()` נוצר כדי לתת נפילה רכה, אבל שבע קריאות עדיין קוראות ל-API הגולמי — חלקן **בזמן render** (`PayrollIntelligence.tsx:43`), כלומר קריסת מסך ולא כשל פעולה. תיקון מכני: להחליף בקריאה ל-`newIdempotencyKey()`. |

**הערה חוצה-פריטים ל-R1-03 (נמדד 03.09.2026):** מתוך המוטציות ב-`client.ts`, **שש** מייצרות מפתח טרי בכל קריאה כברירת מחדל (`startVisaRenewal:222`, `confirmAssistantChecklist:652`, `createProfessionalReview:664`, `transitionProfessionalReview:681`, `createRegulationRule:877`, `transitionRegulationRule:891`) — מפתח חדש בכל לחיצה אינו מגן מפני שליחה כפולה. **שש** דורשות מפתח כפרמטר ולכן בטוחות במבנה, אך ארבעה מהן נקראות עם `crypto.randomUUID()` inline (`CanonicalPayrollIntelligence.tsx:380/454/466/484`) — מה שמבטל את ההגנה בנקודת הקריאה. **שתים־עשרה מוטציות אינן שולחות מפתח כלל**, ובהן `openEmploymentCase`, `uploadCaseDocument`, `createCaseTask`, `createCaseMedication`, `inviteFamilyMember`. **שני מימושים נכונים לחיקוי:** סגירת חודש שכר (`PayrollIntelligence.tsx:43` — `useRef` יציב) וחידוש ויזה (`VisaRenewalSection.tsx:199-205` — מפתח לפי חתימת התשובות, נטבע בשליחה ולא ב-render). האחרון הוא הדפוס הנכון וצריך להפוך לכלל.
...
| ID | נושא | Status | Priority | Evidence / Reference | Notes |
|---|---|---|---|---|---|
| R2-01 | Unified Action Model | NEW | P1 | — | 01.09.2026 — אומת בקוד: אין מודל action משותף. `apps/web/src/product-intelligence.ts` בונה facts לכל משטח בנפרד, ו-`DashboardPage.tsx`, `OpenIssuesPage.tsx` ו-`components/OpenIssuesGlance.tsx` צורכים אותם עצמאית. תלוי ב-R0-04: מודל action אחיד מעל שני מודלי אחסון ייצור את הכפילות שהוא בא למנוע. **03.09.2026 — נספרו שישה מודלים נפרדים** לאותו דבר: `AttentionFact` ו-`ComplianceTimelineItem` (`packages/application/src/product-intelligence.ts:20`, `:32`), `HealthFactor` (`:186` — provenance משלו), `OpenIssue` (`OpenIssuesGlance.tsx:6` — `severity: urgent\|soon\|ok`), `MvpTask` (`mvp-storage.ts:589`), `CaseHealthResponse.factors` (`client.ts:557-571` — **מוצהר מחדש בצד הלקוח במקום להיות מיובא מ-`application`**), ו-`Task` של שיתוף הפעולה (`CollaborationPanel.tsx:9`). היחיד שמשותף הוא רכיב ה-**תצוגה** `OpenIssuesGlance`, ואותו הדשבורד כלל אינו משתמש בו. |
| R2-02 | Home — דורש טיפול | PARTIAL | P1 | `apps/web/src/pages/DashboardPage.tsx:318-353` (`section.intelligence-attention`) · `apps/web/src/pages/OpenIssuesPage.tsx` · `apps/web/src/product-intelligence.ts` · `packages/i18n/src/resources/he.json:465` "דורש טיפול" · `DashboardPage.test.tsx:214-221` | 01.09.2026 — קיים בפועל, נגזר מ-facts קנוניים ולא מ-store חדש, ומציג `liability.reminder`. חסר: סדר עדיפות מוכח (אין assertion על סדר), ולא נבדק ברוחבי מובייל (מטריצת הרוחבים כן מכסה את הדשבורד — ראה R4-01). **03.09.2026 — באג שקט שנמצא ותוקן:** ציון הבריאות נשלף עם **מזהה הלקוח הישן** מול API שמפתחו הוא **מזהה התיק** — כל הבקשות החזירו 404, ובהיעדר `.catch` הדחייה נבלעה. כלומר במשך כל התקופה הזו המסך של המשפחה נראה בדיוק כמו תיק שאין בו מה לטפל. תוקן ב-`DashboardPage.tsx:162-163` וב-`OpenIssuesPage.tsx:35-42` — `findCanonicalCase(clientId)` ואז `getCaseHealth(found.id)`, עם `.catch` שמדליק `healthError` ומציג הודעה (`DashboardPage.tsx:402`). **שארית ידועה — ראה R2-08.** |
...
| R2-06 | CareDesk Score — framing אנושי | PARTIAL | P1 | `apps/web/src/pages/DashboardPage.tsx:354-384` · `he.json:472` `healthDisclaimer` — "מדד שלמות המבוסס על המידע בתיק; **אינו אישור לעמידה בדין**" · `DashboardPage.tsx:369` `liability.score` | 01.09.2026 — עלה מ-`NEW`: ההפרדה בין Score ל-Legal Compliance קיימת בטקסט ובכתב-ויתור, כנדרש בשער R2→R3. **הסדר עדיין Score → הסבר**, לא `Text → Explanation → Score` כפי שהפריט מגדיר. **03.09.2026 — מדויק יותר:** ברמת העמוד הסדר **כן** נכון — כרטיס "דורש טיפול" ראשון (`DashboardPage.tsx:330`), כרטיס הציון אחריו (`:388`). הפער הוא בשני מקומות: (1) בתוך כרטיס הציון, הטבעת המספרית (`:393-396`) מקדימה את הכותרת וההסבר (`:399-400`); (2) **שני המשטחים אינם מסכימים** — ב-`OpenIssuesGlance` הציון מוצג לפני כל הסבר (`:56-59` מול `:61-63`) ומעל רשימת הנושאים (`:55` לפני `:66`). יש להכריע על סדר אחד ולהחיל אותו בשניהם. |
| R2-07 | CTA אחד ברור לכל Action | PARTIAL | P1 | `he.json:467` `intelligence.handle` = "לטיפול עכשיו" · `DashboardPage.tsx` (מקטע `attention`) · `he.json:493` `dashboard.nextAction` | 01.09.2026 — עלה מ-`NEW`: קיים CTA יחיד ומנוסח בפריטי "דורש טיפול". לא אומת שהכלל נשמר בכל action ובכל מסך. **03.09.2026 — הכפתור הזה לא עבד.** `factor.actionTarget` מגיע מה-API כנתיב שכבר מושרש באפליקציה (`/cases/{id}#documents`), ועטיפתו ב-`path()` (שמוסיף `/clients/:clientId`) ייצרה כתובת שאינה תואמת אף route — ה-router שלח את המשתמש ל-`/app` בלי הודעת שגיאה. כלומר ה-CTA היחיד והברור לא עשה כלום. תוקן בשני המסכים; **אומת ידנית בייצור על ידי הבעלים.** |
| R2-08 | "לא הצלחנו לאתר את התיק" נראה כמו "אין מה לטפל" | **DEPLOYED** | P1 | `apps/web/src/pages/DashboardPage.tsx:162-169` — רק דחייה מדליקה `healthError`; `found === undefined` הולך בנתיב השקט ומגיע ל-`dashboard.scoreUnavailable` (`:400`) | 03.09.2026 — שארית של תיקון R2-02. כשל רשת מדווח עכשיו כראוי, אבל "התיק לא נמצא" עדיין מוצג כהיעדר ולא כמצב. עבור לקוח שהתיק שלו קיים אך הקישור אליו נשבר, ההבדל הוא בין "הכול בסדר" ל"משהו לא תקין" — בדיוק ההבחנה ש-`TimelinePage` כבר עושה נכון (`'none'` מול `'unavailable'`, `use-case-for-legacy-client.ts`). הדפוס קיים; יש להחיל אותו גם כאן. |
...
| ID | נושא | Status | Priority | Evidence / Reference | Notes |
|---|---|---|---|---|---|
| R3-01 | Owner visible on action | NEW | P1 | — | 01.09.2026 — לא אומת. פריטי "דורש טיפול" ב-`DashboardPage.tsx:318-353` מציגים provenance (מקור) אך לא אחראי. ADR-004 ו-`family-access` מספקים את הנתון; התצוגה אינה קיימת. **03.09.2026 — תיקון לרישום הקודם: הנתון גם אינו קיים במודל.** ל-`HealthFactor` (`product-intelligence.ts:186-197`), ל-`AttentionFact` (`:20-31`), ל-`OpenIssue` (`OpenIssuesGlance.tsx:6-14`) ול-`MvpTask` (`mvp-storage.ts:589`) **אין שדה אחראי כלל**. זה אינו פער תצוגה אלא פער מודל, ולכן תלוי ב-R2-01. המקום היחיד שבו אדם מופיע ליד עבודה הוא `CollaborationPanel.tsx:169-188` (בורר מוקצה למשימה) ו-`ProductCompletionPanel.tsx:343` (`review.assignedTo` — טקסט חופשי). |
| R3-02 | Assign / reassign לפי הרשאות | PARTIAL | P1 | `apps/web/src/pages/case/CollaborationPanel.tsx` + `CollaborationPanel.test.tsx` · `apps/api/src/routes` — `registerFamilyAccessRoutes` (`create-server.ts:175`) · ADR-004 | 01.09.2026 — הקצאה קיימת בפאנל שיתוף הפעולה. אכיפת הרשאות בשרת קיימת (`PgMembershipAuthorizationService`, `container.ts:334`); לא אומת שכל מסלול הקצאה עובר דרכה. **03.09.2026 — אומת:** האכיפה אמיתית ובמקום הנכון — `packages/application/src/collaboration.ts:104` דורש `owner`/`manager` ובודק דייר ופעילות (`:105-107`); המסלולים `PUT /cases/:caseId/responsibilities/:kind` (`wave5.ts:73`) ו-`PUT /cases/:caseId/tasks/:taskId/assignee` (`:100`) עוברים דרכה ודורשים `idempotency-key`. **מה שחסר הוא בצד הלקוח:** הבורר מוצג לכל אחד, וצופה מגלה את החסימה רק כ-403 שנתפס להודעת שגיאה גנרית. בנוסף ההקצאה זמינה **רק** ממסך התיק — לא מהדשבורד, לא מ"נושאים פתוחים" ולא ממסך המשימות, שהם המסכים שבהם המשפחה בפועל רואה מה צריך לעשות. |
...
| R5-05 | Provenance display | PARTIAL | P1 | `apps/web/src/pages/DashboardPage.tsx` — המחרוזת הטכנית הוחלפה ב-`t('valueOrigin.provenance.source')` עם fallback ל-token הגולמי · `ValueOrigin` `provenance={{ source, who, when }}` · "מתי" אמיתי מוצג ב-`PayrollIntelligence.tsx` (תאריך תשלום מ-`payroll_month_close`) וב-`PayrollPage.tsx` (`savedAt` של הוצאה) · בדיקות: `ValueOrigin.test.tsx` (R5-05), `DashboardPage.test.tsx` ("names the source… in Hebrew", "falls back to the raw source token") | 01.09.2026 — נשאר `PARTIAL` **במכוון**. "מקור" ו-"מתי" מוצגים היכן שהנתון כבר נושא אותם. **"מי" אינו קיים באף אחד מהמקורות**: `HealthFactor.provenance` הוא `{sourceType, sourceIds}` בלבד, ו-`payroll_month_close` אינו נושא שחקן. הרכיב משמיט את החלק שאין לו נתון במקום למלא מציין מקום. סגירת הפריט דורשת החלטת מוצר על נשיאת actor — כלומר שינוי נתונים, שאינו ב-scope של סבב תצוגה. **03.09.2026 — תיקון מהותי לרישום הזה: לסגירת חודש שכר יש actor, והוא כבר נשמר במסד.** `closed_by` נכתב בכל סגירה ואינו מוחזר בשאילתה — ראה **R5-08**. כלומר החצי הפיננסי של "מי" ניתן לסגירה **בלי שינוי נתונים ובלי מיגרציה**, בשינוי קריאה בלבד. הטענה "אין 'מי' באף מקור" נשארת נכונה **רק לגורמי הבריאות**, שהם נגזרות מערכת ולא פעולות אדם — ושם ייתכן שהתשובה הנכונה היא להצהיר שהמקור הוא המערכת, ולא לחפש שם. |
| R5-06 | Monthly Close human summary | PARTIAL | P1 | `apps/api/src/routes/canonical-product-intelligence.ts:104` (`POST /cases/:caseId/payroll-month-closes`) · `apps/api/src/product-intelligence/canonical-intelligence-service.ts:115-158` — טרנזקציה אחת: `timeline_event` + `audit_event` + `payroll_month_close`, מאחורי בדיקת תפקיד (`:95`), קיום תיק (`:96-97`) ואידמפוטנטיות (`:100-114`) · חיווט לקוח: `PayrollIntelligence.tsx:118-132` | 01.09.2026 — לא אומת סיכום אנושי. WEB-01 של שורש 5 ("סגירת חודש שכר לא עושה כלום") לא אומת כנסגר בסבב הזה. **03.09.2026 — אומת: הסגירה כן כותבת בפועל, ו-WEB-01 סגור.** שלוש רשומות באותה טרנזקציה. **מה שנשאר חסר הוא הסיכום עצמו:** אחרי הצלחה מתבצע `refreshCloses()` בלבד (`:132`), וההיסטוריה מציגה שורה אחת — `{חודש} — הושלם · שולם {תאריך} · נסגר {תאריך ושעה}` (`:437-443`). **הסכומים נמצאים בתשובה** (`total/baseSalary/additions/deductions`, `client.ts:616-619`) **ואינם מוצגים**. אין פאנל אישור, אין "מה נסגר", אין מי. הערה נוספת: `PayrollIntelligence.tsx:409` — "אישור עובד אינו נתמך עדיין", וזה נכון ומוצהר. |
| R5-07 | Future Cost placement | NEW | P2 | — | 01.09.2026 — לא נבדק בסבב הזה. |
| R5-08 | `closed_by` נכתב למסד ואינו נקרא לעולם | **DEPLOYED** | P1 | `apps/api/src/product-intelligence/canonical-intelligence-service.ts:131` — העמודה נכתבת מתוך `actor.userId` (`:145`) · `:133-137` — פסקת ה-`returning` **משמיטה אותה** · `apps/web/src/api/client.ts:610-621` — ל-`CanonicalPayrollClose` אין `closedBy` | 03.09.2026 — **זהו המפתח לסגירת "מי" של R5-05, וכבר נמצא במסד.** הנתון קיים בכל סגירת חודש מאז שהמסלול נכתב; הוא פשוט אינו יוצא מהשאילתה ואינו קיים בטיפוס. כלומר "אין לנו את הנתון" נכון לגורמי הבריאות, אך **אינו נכון לסגירת חודש**. פעולה: להוסיף `closed_by` ל-`select` ול-`returning`, לפתור לשם תצוגה, ולהוסיף `closedBy` לטיפוס — ואז R5-05 נסגר לפחות למשטח הכסף. שינוי קריאה בלבד, ללא מיגרציה. |
...
| R7-10 | **ייצוא התיק להורדה — המשפחה מחזיקה עותק משלה** | **NEW** | P1 | רעיון הבעלים, 03.09.2026 · תשתית קיימת לשימוש חוזר: `apps/api/src/binder-export-service.ts` (manifest + sha256 + קבלה ב-`binder_export_receipt`, מיגרציה 0031) ו-`apps/api/src/evidence-export-service.ts` — **שניהם מייצרים מניפסט בלבד, ללא תוכן השורות**; `packages/application/src/ports/document-storage.ts:12` לקבצים | 03.09.2026 — **הצעת הבעלים, ואני חושב שהיא נכונה.** ניסוח שלו: "בדיוק כמו אקסל — אם נמחק הקובץ זו אחריות שלך". כפתור אחד שמוריד את התיק כולו: הנתונים הקנוניים + קבצי המסמכים + סיכום קריא. **למה זה חזק:** (1) המשפחה מחזיקה את הרשומות שלה — רשומות העסקה שייתכן שהיא חייבת לשמור בכל מקרה; (2) זה מקטין את התלות בנו כמשמורן היחיד; (3) זה טיעון מכירה, לא רק בקרה — "הנתונים שלך, ואתה יכול לקחת אותם"; (4) זו גם יציאה מסודרת מהמוצר, שמורידה חיכוך בכניסה אליו. **מה שהוא לא עושה, וחשוב לרשום:** האנלוגיה לאקסל נכונה רק אחרי שהמשתמש הוריד בפועל. רובם לא יורידו. לכן **זה תוספת לגיבוי ולא תחליף לו** — הודעה בסגנון "יש לך עותק ולכן אנחנו לא אחראים" תהיה נכונה רק לגבי מי שהוריד. **איני עורך דין:** האם מסירת עותק ללקוח מעבירה או מקטינה אחריות משפטית של מחזיק המידע היא שאלה לייעוץ, ולא הנחה שנבנה עליה מוצר. **היקף מוצע:** ZIP הכולל JSON קנוני לכל התיק, קבצי המסמכים כפי שהם, וסיכום PDF קריא לאדם. **לא לפני** שנושא ההרשאות ותיעוד ה-audit של הייצוא סגורים — ייצוא מלא הוא גם משטח הדלפה. |
| R7-09 | תהליך סיום העסקה — קליטת פירוט התאגיד וחישוב הפער | **NEW** | P1 | `docs/governance/TERMINATION-SETTLEMENT.md` (נוצר 02.09.2026 מהכתבת הבעלים) · אין עדיין קוד | 03.09.2026 — **הוגדר במלואו על ידי הבעלים ומחכה לאישור רשימת הרכיבים לפני מימוש.** הכלל שנקבע: **פירוט סגירת התיק מהתאגיד הוא הקובע** לגבי כל הזכויות הסוציאליות; המערכת מחשבת במקביל; **הפער בין השניים הוא חוב לתשלום לעובד**. זהו הרגע היחיד במוצר שבו נתון חיצוני גובר על החישוב הפנימי, ולכן הוא דורש תהליך מובנה של קליטת נתונים חיצוניים — לא הקלדה חופשית. **ממתין להחלטה:** אישור רשימת הרכיבים (פיצויי פיטורים, תגמולי פנסיה, דמי הבראה, פדיון חופשה, נסיעות, הודעה מוקדמת). לא נכתבה שורת קוד עד לאישור. |
...
|---|---|---|---|---|---|---|
| 2026-09-03 | **סבב תיקונים ממוקד — R1-07, R1-08, R2-08, R5-08** | ענף `feat/typography-for-older-readers` | IN_REVIEW | **לא בוצע** — ממתין למיזוג ולפריסה | (1) **R1-07 נסגר חלקית במכוון**: נעילת השליחה מונעת את שני המסמכים שנוצרו משתי הקשות — זה הפגם שדווח. `uploadCaseDocument` עדיין אינו שולח `idempotency-key`, ולכן תשובה שאבדה ברשת + ניסיון חוזר עדיין יכולים ליצור כפילות בשרת. סגירת החצי הזה דורשת אידמפוטנטיות בצד השרת במסלול ההעלאה, ונשארת פתוחה. (2) **R0-10 לא נגענו בו** — עבודת שרת, Release נפרד. (3) הבדיקות שנכתבו בסבב הזה **לא הורצו מהסביבה הזו** (vitest אינו יכול לרוץ כאן) — הורצו: `tsc --noEmit` ל-web/api/i18n, `eslint`, `prettier --check`, וכל שישה סקריפטי ה-guard. | **HOLD** — נדרשת ריצת CI ירוקה ואימות בייצור לפני `READY_FOR_PRODUCTION`. |
| 2026-09-03 | **PR #122 נמזג ונפרס לייצור** | `2b26616` (merge) ← `5e41c65` · מוזג 13:51 שעון ישראל | **DEPLOYED** | **בוצע — ראיה נצפית.** (1) `main` = `2b26616` ומכיל את `5e41c65`. (2) **אין מיגרציות חדשות** מאז פריסת #120 — `git diff 11ed832..4929bee -- database/migrations packages/db/src/required-migrations.ts` ריק, כלומר לא נדרש שלב מסד. (3) `GET /ready` → `200`, `database: ok`, `privateStorage: ok`, ללא `reasons`. (4) **ה-bundle החי נבדק בשני המארחים** — `care-platform-web.vercel.app` וגם הדומיין הציבורי `www.caredesk-isr.com` — ובשניהם נמצאו ארבע המחרוזות החדשות: `שומר את המסמך…` (R1-07), `סגר/ה` (R5-08), `לא מצאנו תיק העסקה המקושר` (R2-08), `חזרה לתיקי ההעסקה`. שני ה-bundles זהים בגודלם (1,068,644 תווים), כלומר הדומיין הציבורי מגיש את אותה גרסה. | (1) **אף פריט לא סומן `COMPLETE`** — §11 דורש אימות התנהגותי בייצור בידי אדם, ומה שאומת כאן הוא שהקוד נפרס, לא שהמסכים נבדקו ידנית. (2) R0-10 (אין `PATCH`/`DELETE` למסמך) נשאר פתוח. (3) R1-07 נסגר חלקית — אין עדיין `idempotency-key` בהעלאה. (4) שלושה קומיטי תיעוד עדיין לא נדחפו; אינם משפיעים על הייצור. | **GO.** קודמו ל-`DEPLOYED`: R1-07, R1-08, R2-08, R5-08. **הפריט הדחוף הבא הוא R0-12** — אימות חי שהגלאי היומי באמת שולח, כי אחרי החלטת R0-11 הוא מה שמחזיק את חלון השבעה ימים. |
| 2026-09-03 | **Reconciliation שני מול הקוד** (ללא שינוי קוד מוצר) | `4929bee` — ענף `feat/typography-for-older-readers` | DOCUMENTATION | לא רלוונטי — מסמך בלבד | (1) **R0-10 חדש ו-P0**: אין `PATCH`/`DELETE` על מסמך בודד, ולכן מסמך שנערך אחרי הסנכרון נשאר בדפדפן — dual truth במסלול פעיל. (2) **R1-07 חדש ו-P0**: העלאת מסמך ללא הגנת הקשה כפולה וללא `idempotency-key`. (3) R1-01 נמדד מחדש — 16 מסכים, 4 אוצרי מילים. (4) R5-08 — `closed_by` נכתב ואינו נקרא. | **GO** לסבב תיקונים ממוקד: R1-07 → R1-08 → R5-08 → R2-08, לפי סדר עולה של סיכון לנתונים. R0-10 דורש עבודת שרת ולכן Release נפרד. |
| 2026-09-03 | **PR #122 — סבב QA שני: תיקוני מסכים, בוררים לוויזה, סנכרון** | `4929bee` (ענף) — CI ירוק במלואו | READY_FOR_PRODUCTION | **לא בוצע** — ממתין למיזוג ולפריסה | (1) הבדיקות ל-`/cases/new` הצריכו תיקון כי המוצר (נכון) מפנה לקוח שכבר יש לו תיק לתיק הקיים — נוספה בדיקה מפורשת לשני צידי הכלל. (2) fixture ה-e2e לא ענה על שאילתת רשימת התיקים שהמסכים החלו לבצע. | **ראיה התנהגותית מלאה:** כל שמונת ה-jobs ירוקים על `4929bee`, כולל **Playwright 153/153**. |
| 2026-09-02 | **PR #120 — תשעת חוסמי ההשקה** | `11ed832` (merge) · מיגרציות 0046, 0047, 0048 הוחלו | **PRODUCTION_VERIFIED** (חלקי) | **בוצע.** `/ready` החזיר `{"status":"ok"}`; ה-bundle שנפרס אומת כמכיל את המחרוזות החדשות; **הבעלים אימת ידנית** שקישור הפעולה הדחופה עובד ושהעלאת דרכון הצליחה (גורם בריאות 25/25). | (1) R0-10 — מסמך שנערך אחרי הייבוא אינו מגיע לשרת. (2) העלאת המסמכים אומתה לדרכון בלבד; סוגים אחרים לא נבדקו ידנית. (3) הסרת נתיב הכתיבה המקומי לא בוצעה — מותנית באישור הבעלים בעיניו שלו שהנתונים בשרת. | **GO** — אך `COMPLETE` לא ניתן לאף פריט: §11 של נוהל השחרור דורש אימות מלא, ואומתו שני מסלולים בלבד. |
...
|---|---|---|
| 2026-09-03 | Claude | **ארבעה תיקונים מתוך ה-Reconciliation, לפי סדר עולה של סיכון לנתונים.** **R1-07** — `DocumentsPage` נועל את השליחה ב-`useRef` (לא ב-state: state נסגר אסינכרונית, ולחיצה שנייה באותו tick הייתה עוברת), הכפתור מושבת ואומר "שומר את המסמך…", והנעילה משתחררת ב-`finally` כך שקובץ שנדחה אינו נועל את הטופס לתמיד. 3 בדיקות. **R1-08** — שבע קריאות ל-`crypto.randomUUID` הוחלפו ב-`newIdempotencyKey()`, כולל אחת שרצה בזמן render ולכן הפילה מסך שלם מחוץ ל-secure context. **R5-08** — `closed_by` מוחזר מה-select ומה-returning ונפתר לשם תצוגה דרך `list_caredesk_family_members`, פונקציית ה-`security definer` הקיימת. **לא דרך join ל-`app_user`**: לטבלה הזו RLS כפוי ללא policy לתפקיד האפליקציה, וה-join היה מחזיר ריק בשקט. שם שלא ניתן לפתור מוחזר `null` והמסך משמיט את השורה — לעולם לא uuid גולמי על מסך כסף. `closedBy` הוצא מטיפוס הבקשה: הוא נגזר מהמשתמש המאומת ולא יתקבל מהלקוח. 4 בדיקות בשרת + 2 בלקוח. **R2-08** — הדשבורד מבחין בין חיפוש שנכשל לחיפוש שהצליח ולא מצא תיק, עם הודעה שאומרת מפורשות שלא נמחק דבר. 2 בדיקות. **לא נוספה מיגרציה, לא נוצר מקור אמת, ולא שונה שום חישוב.** |
| 2026-09-03 | Claude | **Reconciliation שני מול הקוד.** עודכנו Status/Evidence/Notes ב-13 שורות קיימות (R0-04 בהערת השער, R1-01..R1-06, R2-01/02/06/07, R3-01/02, R5-03/05/06); לא נמחקה אף שורה. **נוספו שישה פריטים חדשים**, כולם מתוך ממצא בקוד ולא מתוך רשימת משאלות: **R0-10** (אין `PATCH`/`DELETE` על מסמך — dual truth במסלול פעיל, P0), **R1-07** (העלאת מסמך ללא הגנת הקשה כפולה וללא `idempotency-key`, P0), **R1-08** (`crypto.randomUUID` גולמי בשבע קריאות, אחת מהן בזמן render), **R2-08** ("לא הצלחנו לאתר את התיק" נראה כמו "אין מה לטפל"), **R5-08** (`closed_by` נכתב ואינו נקרא — המפתח לסגירת "מי" של R5-05 בלי מיגרציה), **R7-09** (תהליך סיום העסקה, מוגדר ומחכה לאישור רשימת רכיבים). **שלושה רישומים קודמים תוקנו כשהתבררו כלא מדויקים:** R1-05 ("אין retry" → יש 18 כפתורים ידניים, אין retry אוטומטי), R2-06 (סדר העמוד דווקא נכון; הפער הוא בתוך הכרטיס ובאי-הסכמה בין שני משטחים), R5-05 ("אין 'מי' באף מקור" → קיים לסגירת חודש, פשוט אינו נקרא). **לא שונה קוד אפליקציה בסבב הזה.** |
| 2026-09-03 | Claude | **תיעוד הפריסה של PR #120 ומצב PR #122.** PR #120 (תשעת חוסמי ההשקה) נפרס לייצור עם מיגרציות 0046–0048; `/ready` תקין; הבעלים אימת ידנית שני מסלולים. PR #122 ירוק במלואו ב-CI (153/153 ב-Playwright) וממתין למיזוג. אף פריט **לא** סומן `COMPLETE` — §11 של נוהל השחרור דורש אימות בייצור, ואומתו שני מסלולים בלבד מתוך התשעה. |
