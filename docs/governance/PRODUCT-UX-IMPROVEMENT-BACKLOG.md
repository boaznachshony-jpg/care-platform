...
| R5-08 | `closed_by` נכתב למסד ואינו נקרא לעולם | **IN_REVIEW** | P1 | `apps/api/src/product-intelligence/canonical-intelligence-service.ts:131` — העמודה נכתבת מתוך `actor.userId` (`:145`) · `:133-137` — פסקת ה-`returning` **משמיטה אותה** · `apps/web/src/api/client.ts:610-621` — ל-`CanonicalPayrollClose` אין `closedBy` | 03.09.2026 — **זהו המפתח לסגירת "מי" של R5-05, וכבר נמצא במסד.** הנתון קיים בכל סגירת חודש מאז שהמסלול נכתב; הוא פשוט אינו יוצא מהשאילתה ואינו קיים בטיפוס. כלומר "אין לנו את הנתון" נכון לגורמי הבריאות, אך **אינו נכון לסגירת חודש**. פעולה: להוסיף `closed_by` ל-`select` ול-`returning`, לפתור לשם תצוגה, ולהוסיף `closedBy` לטיפוס — ואז R5-05 נסגר לפחות למשטח הכסף. שינוי קריאה בלבד, ללא מיגרציה. |
